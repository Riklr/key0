...
**Key2a** is an open-source SDK that lets any API provider publish an A2A-compliant agent, define priced capabilities, and accept crypto payments from client agents — without changing their existing backend.
...
Client agents are **out of scope** — any A2A-compatible agent that can discover an agent card, hold a USDC wallet, and submit a transaction hash can interact with Key2a. No buyer SDK is provided or required.
...
|---|---|---|
| **API Provider** | Owns the product, deploys Key2a alongside their API | No way to monetize agent traffic; signup walls block autonomous clients |
...
| **Open Source Contributor** | Developer adding new payment adapters or integrating Key2a | Needs clear adapter interfaces and well-typed contracts |
...
```
  ...
  (wallet + payment capability assumed — not Key2a's concern)
         │
...
│         Unchanged — Key2a sits in front, injects token check     │
└──────────────────────────────────────────────────────────────────────┘
```
...
**Output (challenge not found)**: `Key2aError` `CHALLENGE_NOT_FOUND`

**Output (challenge expired)**: `Key2aError` `CHALLENGE_EXPIRED`

**Output (chain mismatch)**: `Key2aError` `CHAIN_MISMATCH`

**Output (amount mismatch)**: `Key2aError` `AMOUNT_MISMATCH`

**Output (tx not confirmed)**: `Key2aError` `TX_UNCONFIRMED`

**Output (already redeemed)**: `Key2aError` `TX_ALREADY_REDEEMED`
...
Key2a provides middleware for protecting seller API routes:

```typescript
// Express
import { validateAccessToken } from "@riklr/key2a/express";
...
const payload = await validateKey2aToken(authHeader, { secret });
```
...
Step 6: Mount the agent router
  app.use(key2aRouter({ config, adapter, store, seenTxStore }))
  // Auto-serves:
  //   GET  /.well-known/agent.json
...
Key2a is a **self-hosted open-source SDK**. There is no central registry or SaaS dashboard. The seller runs the agent runtime inside their own infrastructure alongside their existing API.
...
| Package | Single package `@riklr/key2a` with framework subpath exports | Simple install, tree-shakeable |
