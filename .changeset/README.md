...
This folder is managed by [`@changesets/cli`](https://github.com/changesets/changesets). It tracks unreleased changes and drives versioning + changelog generation for `@key0ai/key0`.
...
| Artifact | Target |
|---|---|
| npm package | `@key0ai/key0` on npmjs.com |
| Docker image | `key0ai/key0` on Docker Hub (tagged by semver + `latest`) |
...
3. Builds and pushes the Docker image to `key0ai/key0` (only runs if npm publish succeeds)
