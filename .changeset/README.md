...
This folder is managed by [`@changesets/cli`](https://github.com/changesets/changesets). It tracks unreleased changes and drives versioning + changelog generation for `@riklr/key2a`.
...
| Artifact | Target |
|---|---|
| npm package | `@riklr/key2a` on npmjs.com |
| Docker image | `riklr/key2a` on Docker Hub (tagged by semver + `latest`) |
...
3. Builds and pushes the Docker image to `riklr/key2a` (only runs if npm publish succeeds)
