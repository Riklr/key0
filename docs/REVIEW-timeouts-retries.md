# Key2a — Timeout & Retry Handling Review

This document audits every async operation in the Key2a SDK for timeout protection, retry behavior, and failure-mode gaps. Each finding includes the file, the risk, and a recommended fix.
...
try {
  ...
  console.error("[Key2a] Failed to mark DELIVERED, returning grant anyway:", err);
...
this.config.onPaymentReceived(grant).catch((err: unknown) => {
  console.error("[Key2a] onPaymentReceived hook error:", err);
});
```
...
**Problem**: Neither the store constructors nor `createKey2a()` verify that the Redis connection is healthy. If Redis is misconfigured or down, the first real request will fail with a cryptic ioredis error.
...
```ts
const lockKey = `key2a:settle-lock:${privateKey.slice(0, 10)}`;
```
...
```ts
const lockKey = `key2a:settle-lock:${gasAccount.address}`;
```
...
**Problem**: When a PAID record's challenge hash expires after 7 days (Redis TTL), the entry in the `key2a:paid` sorted set is never removed. The refund cron calls `findPendingForRefund`, gets the challengeId from the sorted set, but `store.get()` returns null. The entry is silently skipped (line 270) but **remains in the sorted set forever**.
