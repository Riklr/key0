...
| Option              | Type                    | Default           | Description                                   |
| ------------------- | ----------------------- | ----------------- | --------------------------------------------- |
| `store`             | `IChallengeStore`       | required          | The same store passed to `createKey2a`    |
...
| Key                                      | TTL                                       |
| ---------------------------------------- | ----------------------------------------- |
| `key2a:challenge:{id}` (hash)        | **7 days** (set at creation)              |
| `key2a:request:{requestId}` (string) | `challengeTTLSeconds` (900s default)      |
| `key2a:paid` (sorted set)            | no expiry — members removed on transition |


On `PAID → DELIVERED`, the SDK immediately calls `EXPIRE key2a:challenge:{id} 43200` to shorten the hash key TTL to **12 hours**. No background job needed — the TTL reset happens inside the same transition call.
...
const { requestHandler } = createKey2a({
  store,
  seenTxStore,
...
Uses a sorted set `key2a:paid` (member = `challengeId`, score = `paidAt` epoch ms) maintained alongside the hash:

- On `PENDING → PAID`: `ZADD key2a:paid <paidAt_ms> <challengeId>`
- On `PAID → anything`: `ZREM key2a:paid <challengeId>`

Query:

```
ZRANGEBYSCORE key2a:paid 0 <(now - minAgeMs)>
```
