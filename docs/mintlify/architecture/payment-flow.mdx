...
description: "The full Key0 payment lifecycle: challenge issuance, on-chain settlement, token delivery, and the two endpoints that feed into the shared ChallengeEngine."
---
...
The Express integration mounts both `GET /discovery` and `POST /x402/access`. The request determines which of four cases applies.

### Case 1a: Discovery (GET /discovery)

Call `GET /discovery` to browse all available plans. No PENDING record is created — this is a pure pricing query.
...
`POST /x402/access` with an empty body or no `planId` now returns an HTTP **400** error pointing to `GET /discovery`. This is not the discovery endpoint — it is an error response.
...
```json
{
  "error": "Please select a plan from the discovery API response to purchase access. Endpoint: GET /discovery"
}
```
...
| `x-a2a-extensions` | Client to Server | Presence check | On `POST /x402/access` (Express only): routes to A2A JSON-RPC handler instead of x402 HTTP flow |
