  ...
  "skills": [
    {
      "id": "discover-plans",
      "name": "Discover Plans",
      "description": "Browse available plans and pricing for Acme Photo API. Returns the product catalog with plan IDs, prices (USDC on base-sepolia), wallet address, and chain ID. GET https://api.acme.com/discovery to discover plans.",
      "tags": ["discovery", "catalog", "x402"],
      "examples": [
        "GET https://api.acme.com/discovery"
      ]
    },
    {
      "id": "request-access",
      ...
      "workflow": [
        "1. Call GET /discovery to browse available plans and note the planId",
...
A free discovery skill. Clients call this (or GET to `/discovery`) to browse the plan catalog. Returns plan IDs, prices, wallet address, and chain ID.
