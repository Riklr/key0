...
Create a `server.ts` file. Key0 mounts as middleware on your existing server -- it adds the agent card endpoint (`/.well-known/agent.json`), the plan discovery endpoint (`/discovery`), the payment endpoint (`/x402/access`), and leaves your existing routes untouched.
...
The first call returns the A2A agent card. The second returns all available plans and pricing — use the `planId` values from here when requesting access. The third call creates a PENDING challenge containing the payment amount, destination wallet, and chain ID. A client agent uses this to submit payment on-chain and receive an `AccessGrant`.
