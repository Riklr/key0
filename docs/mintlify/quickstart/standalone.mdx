...
The discovery call returns all available plans and pricing — start here to find a valid `planId`. The access request returns an x402 challenge containing the payment amount and destination wallet. An agent completes the flow by paying on-chain and submitting proof.
