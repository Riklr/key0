# Key2a Payment Flow — Complete Lifecycle
...
Key2a uses a **two-phase payment flow**: the client first requests access (receiving a payment challenge), then signs an EIP-3009 authorization off-chain. The server (or a facilitator) settles on-chain. The client never sends a transaction directly.
...
`RedisChallengeStore` exposes a `healthCheck()` method that sends a `PING` to Redis and throws if the response is not `PONG`. This is **not called automatically** by `createKey2a()` — callers should invoke it at startup for fail-fast behavior:

```ts
const { engine, store } = createKey2a(config);
await store.healthCheck(); // throws if Redis is unreachable
```

### Key Naming Convention

All keys use the prefix `key2a` (configurable via `keyPrefix`).

### 1. Challenge Record Hash — `key2a:challenge:{challengeId}`
...
### 2. Request Index — `key2a:request:{requestId}`
...
```
KEY:   key2a:request:550e8400-e29b-...
VALUE: http-a1b2c3d4-...
TTL:   900s (challengeTTLSeconds)
```

### 3. Seen Transaction Set — `key2a:seentx:{txHash}`

A simple `SET NX` key for **double-spend prevention**. Maps `txHash -> challengeId`.

```
KEY:   key2a:seentx:0x1234abcd...
VALUE: http-a1b2c3d4-...
TTL:   7 days (604,800s)
```

### 4. Paid Set (Sorted Set) — `key2a:paid`

A Redis Sorted Set tracking PAID records for the refund cron.

```
ZADD key2a:paid <paidAt_epoch_ms> <challengeId>
```
...
- `KEYS[1]` = `key2a:challenge:{challengeId}`
- `KEYS[2]` = `key2a:paid`
...
Middleware (`validateAccessToken`) verifies JWT, attaches decoded claims to request as `req.key2aToken`.
...
```
key2a:challenge:http-a1b2c3d4-...  (HASH, TTL 7d)
...
key2a:request:550e8400-...  (STRING, TTL 900s)
  = http-a1b2c3d4-...
```
...
```
key2a:challenge:http-a1b2c3d4-...  (HASH, TTL reset to 12h)
  state       = DELIVERED
  txHash      = 0xabcdef...
...
key2a:seentx:0xabcdef...  (STRING, TTL 7d)
  = http-a1b2c3d4-...

key2a:paid  (SORTED SET)
...

{
  ...
  "accepts": [ ... ],
  "extensions": {
    "key2a": { "inputSchema": { ... }, "outputSchema": { ... }, "description": "..." }
  },
  "challengeId": "http-a1b2c3d4-...",
  "error": "Payment required"
}
...
POST {basePath}/jsonrpc
 ...
 x402HttpMiddleware
        ...
        │       YES → pass through to A2A SDK → Key2aExecutor (Transport 3)
        │
...
When a native A2A client sends `X-A2A-Extensions` header, the middleware passes through to the A2A SDK, which routes to `Key2aExecutor`.
...
- **Express**: `req.key2aToken`
- **Hono**: `c.set("key2aToken", payload)`
- **Fastify**: `request.key2aToken`
...
1. **Query**: `store.findPendingForRefund(minAgeMs)` runs `ZRANGEBYSCORE key2a:paid 0 <cutoff>` to find PAID records older than `minAgeMs`, filtering for state=PAID, `fromAddress` set, and `accessGrant` NOT set (records with `accessGrant` were successfully issued but the DELIVERED transition failed — they should not be refunded). Ghost entries (sorted set member but expired hash) are cleaned up via `ZREM`. Records without `fromAddress` trigger a warning log for manual intervention.
...
### Key2aErrorCode (thrown by ChallengeEngine)
...
2. **Double-spend is impossible** — `SET NX` on `key2a:seentx:{txHash}` ensures one txHash maps to exactly one challenge; if `markUsed` fails, the PAID state is rolled back to PENDING
