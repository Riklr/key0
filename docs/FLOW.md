...
The refund cron handles PAID records that were never DELIVERED (e.g., `fetchResourceCredentials` failed or the client disappeared). The cron is **not built into Key0** — the `IChallengeStore.findPendingForRefund()` method and state transitions are provided for external cron implementations.
