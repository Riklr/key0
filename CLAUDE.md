...
Package name: `@riklr/key2a` (single package, not a monorepo despite README references to packages).
...
`createSharedSecretAuth`, `createJwtAuth`, `createOAuthAuth` — service-to-service auth strategies for outbound requests from client agents. `RemoteVerifier` and `RemoteTokenIssuer` wrap remote Key2a endpoints.
