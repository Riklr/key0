# Claude Code Best Practices — Key2a SDK
...
- `Key2aError` assertions check both `.code` and `.httpStatus`
...
The 5 security invariants that protect Key2a's payment flow. Both agents preload this skill.
...
Contains: imports, `makeConfig()`/`makeEngine()`/`makeRequest()` factory patterns, injectable clock for time-travel testing, `TestChallengeStore`/`TestSeenTxStore` from `test-utils/stores.js`, `Promise.all` + filter-Boolean concurrency assertions, `Key2aError` `.code`/`.httpStatus` assertions, `MockPaymentAdapter.setVerifyResult()`.
