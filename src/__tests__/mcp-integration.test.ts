...
// ---------------------------------------------------------------------------
// discover_products tool
// ---------------------------------------------------------------------------

describe("createMcpServer — discover_products tool", () => {
	test("returns product catalog with agent metadata", async () => {
		...
		const result = (await callTool(server, "discover_products", {})) as {
		...
		expect(catalog.products[0].tierId).toBe("basic");
		expect(catalog.products[0].amount).toBe("$0.99");
		expect(catalog.products[1].tierId).toBe("premium");
	});
...
describe("createMcpServer — extractPaymentFromMeta (via request_access)", () => {
	test("no _meta → treated as no payment, returns PaymentRequired result", async () => {
		...
		const result = (await callTool(
			server,
			"request_access",
			{ tierId: "basic", resourceId: "default" },
	...
	test("_meta with malformed x402/payment object → propagates INVALID_REQUEST Key2aError", async () => {
		const { engine, config } = makeEngine();
		const server = createMcpServer(engine, config);
		...
		// so Key2aError propagates directly as a thrown error.
		...
		expect(err).toBeInstanceOf(Key2aError);
		expect((err as Key2aError).code).toBe("INVALID_REQUEST");
		expect((err as Key2aError).httpStatus).toBe(400);
	});
...
describe("createMcpServer — payment-failed error re-wrapping", () => {
	test("PAYMENT_FAILED error returns isError:true with structuredContent overriding error field", async () => {
		settlePaymentImpl = async () => {
			throw new Key2aError("PAYMENT_FAILED", "insufficient_funds", 402);
		};
	...
	test("402 httpStatus error (non-PAYMENT_FAILED code) is also re-wrapped as payment-required", async () => {
		settlePaymentImpl = async () => {
			throw new Key2aError("PAYMENT_FAILED", "nonce_consumed", 402);
		};
	...
	test("structuredContent is a plain object spread (not nested under error key)", async () => {
		settlePaymentImpl = async () => {
			throw new Key2aError("PAYMENT_FAILED", "signature_mismatch", 402);
		};
	...
	test("non-PAYMENT_FAILED Key2aError is returned as generic isError result with toJSON", async () => {
		settlePaymentImpl = async () => {
			throw new Key2aError("CHAIN_MISMATCH", "Chain mismatch on settlement", 400);
		};
	...
	test("non-Key2aError is re-thrown (not swallowed)", async () => {
		settlePaymentImpl = async () => {
