...
function createMockSql() {
	...
	const sql = (strings: any, ...values: any[]): any => {
		...
		if (
			...
			query.includes("where challenge_id")
		) {
			const tableName = values[0];
		...
		// SELECT * WHERE state = 'PAID'
			...
			query.includes("select * from") &&
			query.includes("where state = 'paid'")
		) {
			const tableName = values[0];
			result = findRows(
				tableName,
		...
		// UPDATE (for transition and cleanup)
			...
			count = updateRows(
				tableName,
				(r) => r.challenge_id === challengeId && r.state === fromState && (r.deleted_at === null || r.deleted_at === undefined),
				updates,
			);
			result = [];
		}
...
describe("PostgresChallengeStore", () => {
	...
	test("uses custom tablePrefix", async () => {
		...
		expect((sql as { _tables: Map<string, Row[]> })._tables.has("myapp_challenges")).toBe(
			true,
		);
	});
...
describe("PostgresSeenTxStore", () => {
	...
	test("uses custom tablePrefix", async () => {
		...
		await store.markUsed(TX_HASH, "challenge-1");
		expect((sql as { _tables: Map<string, Row[]> })._tables.has("custom_seen_txs")).toBe(
			true,
		);
	});
});
