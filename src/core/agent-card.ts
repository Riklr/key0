	...
	const skills: AgentSkill[] = [
		{
			...
			description: [
				...
				`Returns the product catalog with tier IDs, prices (USDC on ${networkName}), wallet address, and chain ID.`,
				`POST to ${baseUrl}/x402/access with an empty body or without tierId to discover products.`,
			].join(" "),
			tags: ["discovery", "catalog", "x402"],
			examples: [
				`POST ${baseUrl}/x402/access with empty body {}`,
				`Or call without tierId to get 402 response with product catalog`,
			],
		},
		{
			id: "request-access",
			name: "Request Access",
			description: [
				`Purchase access to a ${config.agentName} product tier via x402 payment on ${networkName}.`,
				`First call discover-products to get available tiers.`,
				`Then POST to ${baseUrl}/x402/access with tierId and requestId to initiate purchase.`,
			...
			examples: [
				`POST ${baseUrl}/x402/access with { tierId: "<tier-id>", requestId: "<uuid>", resourceId: "default" }`,
