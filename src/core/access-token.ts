...
export class AccessTokenIssuer {
	...
	async verify(token: string): Promise<TokenClaims & { iat: number; exp: number }> {
		if (this.algorithm === "RS256") {
			...
			// In practice, verification should use the public key separately via validateKey2aToken
			throw new Error("RS256 verification requires public key, use validateKey2aToken instead");
		}
