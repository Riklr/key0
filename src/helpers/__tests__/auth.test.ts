...
describe("signedJwtAuth", () => {
	...
	test("calls issuer.sign with sub: 'key2a-service'", async () => {
		...
		expect(claims.sub).toBe("key2a-service");
	});
...
describe("oauthClientCredentialsAuth", () => {
	...
	test(
		"throws Key2aError with code INTERNAL_ERROR on non-ok OAuth response",
		withFetch(
	...
	test(
		"throws Key2aError with code INTERNAL_ERROR on invalid OAuth response (missing access_token)",
		withFetch(
