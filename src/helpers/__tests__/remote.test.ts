...
describe("createRemoteResourceVerifier", () => {
	...
	test(
		"throws Key2aError with code RESOURCE_VERIFY_TIMEOUT and httpStatus 504 on abort",
		withFetch(
			mock(async () => {
...
describe("createRemoteTokenIssuer", () => {
	...
	test(
		"throws Key2aError with code TOKEN_ISSUE_FAILED and httpStatus 502 on non-2xx response",
		withFetch(
	...
	test(
		"throws Key2aError with code TOKEN_ISSUE_FAILED when response missing 'token' field",
		withFetch(
	...
	test(
		"throws Key2aError with code TOKEN_ISSUE_TIMEOUT and httpStatus 504 on abort",
		withFetch(
			mock(async () => {
				const err = new Error("aborted");
				err.name = "AbortError";
	...
	test(
		"throws Key2aError with code TOKEN_ISSUE_FAILED and httpStatus 502 on network error",
		withFetch(
			mock(async () => {
				throw new Error("ECONNREFUSED");
			}),
