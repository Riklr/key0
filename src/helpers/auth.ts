 ...
 * Useful when Key2a is configured with RS256 keys and the backend has the public key.
 *
 * @example
 * ```typescript
...
export function oauthClientCredentialsAuth(config: {
	...
	return async () => {
		...
		try {
			...
			throw new Key2aError("INTERNAL_ERROR", `Failed to authenticate via OAuth: ${msg}`, 500);
		}
	};
}
