...
function resolveTier(planId: string, config: SellerConfig): Plan {
	const tier = config.plans.find((p) => p.planId === planId);
	if (!tier) {
		throw new Error(
			`[Key0 PayPerRequest] Plan "${planId}" not found in config.plans. ` +
				`Available: ${config.plans.map((p) => p.planId).join(", ")}`,
		);
	}
	return tier;
}
...
/** Resolve deps from key0Router / key0App shared deps */
function resolveFromRouter(
	planId: string,
	deps: PayPerRequestDeps,
	options?: PayPerRequestOptions,
): ResolvedDeps {
	const tier = resolveTier(planId, deps.config);
	const fetchResource =
		options?.fetchResource ??
		(options?.proxyTo ? proxyToFetchResource(options.proxyTo) : undefined);

	if (fetchResource && !deps.store) {
		console.warn(
			`[Key0 PayPerRequest] WARNING: plan "${planId}" is in standalone gateway mode ` +
				"(fetchResource/proxyTo) without a store. If fetchResource throws after on-chain " +
				"settlement, the payer cannot be automatically refunded. Pass a store for recovery.",
		);
	}
...
export function mergePerRequestRoutes(
	...
	for (const plan of plans) {
		const configRoutes = plan.routes ?? [];
...
export function createExpressPayPerRequest(
	...
	const deps = resolveFromRouter(planId, routerDeps, options);
