...
This example demonstrates how to deploy Key2a as a **separate service** that communicates with your backend via HTTP.
...
2. **Configure environment variables:**
   - `KEY2A_WALLET_ADDRESS`: Your wallet to receive USDC payments
...
Key2a issues its own JWT tokens. Your backend validates them using `@key2a/validator`.
...
app.use("/api", async (req, res, next) => {
  try {
    const payload = await validateKey2aToken(
      req.headers.authorization,
      { secret: process.env.KEY2A_ACCESS_TOKEN_SECRET }
    );
    req.key2aToken = payload;
    next();
  } catch (err) {
...
Your backend issues custom tokens (API keys, session IDs, etc.). Key2a calls your backend to get the token.
...
2. **Set public URL:**
   ```bash
   KEY2A_PUBLIC_URL=https://key2a.yourcompany.com
   ```

3. **Use mainnet:**
   ```bash
   KEY2A_NETWORK=mainnet
   ```
