...
1. **No planId** → discovery 402: returns all plans in the `accepts` array with `planId` in each plan's `extra`. No PENDING record is created. Also validates the `payment-required` header, `www-authenticate` header, and `key0` extensions (inputSchema, outputSchema).
