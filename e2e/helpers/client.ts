/**
 * E2eTestClient: drives the Key2a Docker payment flow.
 *
...
export class E2eTestClient {
	constructor(
		private readonly key2aUrl: string,
	...
	async requestAccess(opts: {
		...
		const res = await fetch(`${this.key2aUrl}/x402/access`, {
			method: "POST",
	...
	async fetchAgentCard(): Promise<AgentCard> {
		const res = await fetch(`${this.key2aUrl}/.well-known/agent.json`);
