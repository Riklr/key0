...
1. **Discovery (optional)** — Client calls `GET /discovery` to receive a 200 response listing all available plans and pricing. Alternatively, POST to `/x402/access` with no body to receive a 402 with the same catalog. No `PENDING` record is created either way.
